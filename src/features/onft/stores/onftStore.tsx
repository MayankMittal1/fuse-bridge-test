...
import {isEqual, uniq, uniqBy, without} from 'lodash-es';
...
import {OnftBridgeStrategy} from '../sdk/strategy/OnftBridgeStrategy';
...
class OnftBridgeStore {
  ...
  get chains(): ChainId[] {
    return uniq(this.collections.flatMap((c) => c.contracts.map((c) => c.chainId)));
  }
  ...
  get strategy(): OnftBridgeStrategy | undefined {
    const {srcNftContract: srcNft} = this;
    if (!srcNft) return undefined;
    return this.strategies.find((s) => s.supports(srcNft));
  }
  ...
  updateMessageFee() {
    ...
    const {srcChainId, dstChainId, strategy, adapterParams} = this;
    ...
    this.promise.messageFee = fromPromise(
      strategy.getMessageFee(assets, dstChainId, adapterParams),
    );
  }
  updateAssets = flow(
    function* (this: OnftBridgeStore, onftContract: OnftContract, address: string) {
      const strategy = this.strategies.find((s) => s.supports(onftContract));
      if (!strategy) return;
      const assets: OnftTokenAmount[] = yield strategy.getAssets(onftContract, address);
  ...
  updateExtraGas() {
    ...
    const {strategy, srcNftTokens, dstChainId} = this;
    if (!srcNftTokens || srcNftTokens.length === 0) return;
    if (!dstChainId) return;
    if (!strategy) return;
    this.promise.extraGas = fromPromise(strategy.getExtraGas(srcNftTokens, dstChainId));
  }
  ...
  addStrategies(strategies: OnftBridgeStrategy[]): void {
  ...
  bridge = flow(
    function* (this: OnftBridgeStore) {
      try {
        ...
        const isApproved: Awaited<ReturnType<typeof strategy['isApproved']>> =
          yield strategy.isApproved(srcTokens, srcAddress);
        if (!isApproved) {
          this.isApproving = true;
          const approval: Awaited<ReturnType<typeof strategy['approve']>> = yield strategy.approve(
            srcTokens,
          );
        ...
        this.isSigning = true;
        const response: Awaited<ReturnType<typeof strategy.transfer>> = yield strategy.transfer(
          input,
        );
  ...
  updateInflight = flow(
    function* (this: OnftBridgeStore) {
      ...
      for (const strategy of this.strategies) {
        const batch: Awaited<ReturnType<typeof strategy['getInflight']>> =
          yield strategy.getInflight(address);
        inflight.push(...batch);
      }
...
export function initOnftStore() {
  ...
  const updateInflight = async () => {
    ...
    const {strategies, srcAddress} = onftStore;
    if (!srcAddress) return;

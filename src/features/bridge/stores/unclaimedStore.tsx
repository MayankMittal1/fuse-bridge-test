...
import {BridgeStrategy} from '@/bridge/sdk/strategy/BridgeStrategy';
...
class UnclaimedStore {
  ...
  protected getStrategy(currency: Currency) {
    return bridgeStore.strategies.find((s) => s.supportsClaim(currency));
  }
  ...
  updateUnclaimedBalance = flow(
    function* (this: UnclaimedStore, currency: Currency, address: string) {
      ...
      console.log('unclaimed', amount.toExact(), currency.symbol);
  ...
  claim = flow(function* (this: UnclaimedStore, currency: Currency) {
    const provider = this.getStrategy(currency);
    assert(provider);
    try {
      ...
      const claimingAmount: Awaited<ReturnType<BridgeStrategy['getUnclaimed']>> =
        yield provider.getUnclaimed(currency, wallet.address);
...
export function initUnclaimedStore(unclaimedStore: UnclaimedStore) {
  function updateAptosBalance() {
    ...
    const strategies = unclaimedStore.strategies.slice();

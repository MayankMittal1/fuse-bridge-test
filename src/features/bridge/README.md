...
To configure a bridge you will need to register currencies that will be available for transfer via `bridgeStore.addCurrencies` and set strategies that will support source/destination pairs via `bridgeStore.addStrategy`.
...
Once registered, your form will be populated with the currency options and pull balances from connected wallets. To enable bridging, continue to the [Strategies](#Strategies) section, and register at least one strategy that supports a pair of source and destination tokens from your currencies configuration.
...
After registering currencies your bridge will need to add at least one [`BridgeStrategy`](sdk/strategy/BridgeStrategy.ts) via `bridgeStore.addStrategy` to bootstrap a working bridge. The `BridgeStrategy` interface is designed to abstract all of the blockchain interactions from the `bridgeStore`.

Three selector methods in the interface will return true when the combination of source and destination currencies agree with the implementation of that strategy. An example would be when the same OFT is selected on a source and destination newtwork pair, the [`BridgeStrategy__oft_evm`](sdk/strategy/BridgeStrategy__oft_evm.ts) strategy will have `supportsTransfer === true`, and that strategy will be selected by the `bridgeStore` to support and execute the transaction.

The default application includes the following strategies out of the box which can support any combination of the included currencies.

- `BridgeStrategy__bridge_evm`
- `BridgeStrategy__bridge_aptos`
- `BridgeStrategy__oft_evm`
- `BridgeStrategy__oft_aptos`
- `BridgeStrategy__wrapped_evm`

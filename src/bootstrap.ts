...
import {
  OftBridgeApiFactory__aptos,
  OftBridgeApiFactory__evm,
  OftBridgeConfig,
} from '@layerzerolabs/ui-bridge-oft';
import {
...
import {Currency, getChainStage, isAptosChainId, isEvmChainId, Token} from '@layerzerolabs/ui-core';
import {BalanceProvider__evm} from '@layerzerolabs/ui-erc20-sdk';
import {DstConfigProvider__evm, ProviderFactory} from '@layerzerolabs/ui-evm';
import {
  StargateBridge__evm,
  StargateSDK,
...
import {BridgeStrategy__bridge_aptos} from '@/bridge/sdk/strategy/BridgeStrategy__bridge_aptos';
import {BridgeStrategy__bridge_evm} from '@/bridge/sdk/strategy/BridgeStrategy__bridge_evm';
import {BridgeStrategy__oft_aptos} from '@/bridge/sdk/strategy/BridgeStrategy__oft_aptos';
import {BridgeStrategy__oft_evm} from '@/bridge/sdk/strategy/BridgeStrategy__oft_evm';
import {BridgeStrategy__stargate_evm} from '@/bridge/sdk/strategy/BridgeStrategy__stargate_evm';
import {BridgeStrategy__stargate_wrapper_evm} from '@/bridge/sdk/strategy/BridgeStrategy__stargate_wrapper_evm';
import {BridgeStrategy__wrapped_evm} from '@/bridge/sdk/strategy/BridgeStrategy__wrapped_evm';
...
import {OnftBridgeStrategy__evm} from '@/onft/sdk/strategy/OnftBridgeStrategy__evm';
...
export async function bootstrap(lzAppConfig: AppConfig, providerFactory: ProviderFactory) {
  ...
  if (lzAppConfig.bridge.stargate) {
    ...
    if (partnerConfig?.feeCollector) {
      ...
      bridgeStore.addStrategy(new BridgeStrategy__stargate_evm(api));
    } else {
      const api = new StargateBridge__evm(providerFactory, sdk);
      bridgeStore.addStrategy(new BridgeStrategy__stargate_evm(api));
    }

    bridgeStore.addCurrencies(config.pools.map((p) => p.currency));
  ...
  for (const wrappedAssetConfig of lzAppConfig.bridge.wrappedToken) {
    ...
    for (const api of apis) {
      bridgeStore.addStrategy(new BridgeStrategy__wrapped_evm(api));
    }
  }
  ...
  for (const aptosBridgeConfig of lzAppConfig.bridge.aptos) {
    ...
    bridgeStore.addStrategy(new BridgeStrategy__bridge_aptos(api.aptos));
    bridgeStore.addStrategy(new BridgeStrategy__bridge_evm(api.evm));
  }
  ...
  for (const collection of lzAppConfig.bridge.onft) {
    ...
    if (standard === OnftStandard.ERC1155) {
      ...
      onftStore.addCollections([collection]);
      onftStore.addStrategies([new OnftBridgeStrategy__evm(api)]);
  ...
  if (typeof window !== 'undefined') {
    Object.assign(window, {app: {onftStore, bridgeStore, walletStore, transactionStore, uiStore}});
    // Todo:
  ...
  function addEvmOft(oftConfig: OftBridgeConfig) {
    if (oftConfig.tokens.some((t) => isEvmChainId(t.chainId))) {
      ...
      bridgeStore.addStrategy(new BridgeStrategy__oft_evm(api));
    }
  }
  ...
  function addAptosOft(oftConfig: OftBridgeConfig) {
    if (oftConfig.tokens.some((t) => isAptosChainId(t.chainId))) {
      ...
      const api = factory.create(oftConfig);
      bridgeStore.addStrategy(new BridgeStrategy__oft_aptos(api));
    }
  }
  ...
  function addStargateWrapperOft(
    ...
    bridgeStore.addStrategy(new BridgeStrategy__stargate_wrapper_evm(api));
  }
}

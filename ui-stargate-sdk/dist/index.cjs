...
var sgSdk = require('@layerzerolabs/sg-sdk');
var uiBridgeSdk = require('@layerzerolabs/ui-bridge-sdk');
var uiEvm = require('@layerzerolabs/ui-evm');
var assert = require('assert');
var ethers = require('ethers');
var uiErc20Sdk = require('@layerzerolabs/ui-erc20-sdk');
...
function toConfig(input) {
  ...
  const pools = input.pools.map((json) => {
    ...
    const currency = json.poolId === 13 ? uiCore.getNativeCurrency(chainId) : token;
    const pool = {
      poolId,
      chainId,
      token,
  ...
  return {
    ...
    router: (_c = input.router) != null ? _c : sgSdk.ROUTER_ADDRESS,
    routerEth: (_d = input.routerEth) != null ? _d : sgSdk.ROUTER_ETH_ADDRESS,
    disabled: (_e = input.disabled) != null ? _e : []
  };
}
function toToken({ address, chainId, decimals, symbol, name }) {
  return new uiCore.Token(chainId, address, decimals, symbol, name);
}
...
var StargateBridge__evm = class {
  constructor(providerFactory, sdk) {
    ...
    this.getFeeLibraryAddress = pMemoize__default.default(
      async (lpToken) => {
    ...
    this.erc20 = new uiErc20Sdk.ERC20__api(providerFactory);
  }
  async getAllowance(currency, address) {
    if (uiCore.isNativeCurrency(currency))
      return uiCore.CurrencyAmount.fromRawAmount(currency, uiCore.MaxUint256);
  ...
  async approve(amount) {
    ...
    assert__default.default(uiCore.isToken(srcCurrency), "Not a token");
  ...
  supportsTransfer(srcCurrency, dstCurrency) {
    if (!uiCore.isEvmChainId(srcCurrency.chainId))
      return false;
  ...
  supportsClaim(currency) {
    if (!uiCore.isEvmChainId(currency.chainId))
      return false;
    const pools = this.sdk.getAllPools();
  ...
  async getUnclaimed(currency, address) {
    return uiCore.CurrencyAmount.fromRawAmount(currency, 0);
  }
  claim(currency) {
  ...
  async transferEth(input) {
    ...
    assert__default.default(srcPool.poolId === 13, "eth");
    assert__default.default(dstPool.poolId === 13, "eth");
    ...
    const amountLD = uiCore.toBigNumber(input.amount);
    const minAmountLD = uiCore.toBigNumber(input.minAmount);
    const value = uiCore.toBigNumber(input.amount.add(input.fee.nativeFee));
  ...
  async getMessageFee(srcCurrency, dstCurrency, adapterParams) {
    ...
    const payload = "0x";
    const lzTxParams = this.lzTxParams(adapterParams);
    const native = uiCore.getNativeCurrency(srcChainId);
    ...
    return {
      nativeFee: uiCore.toCurrencyAmount(native, nativeFee),
      zroFee: uiCore.toCurrencyAmount(native, lzFee)
    };
  }
  lzTxParams(adapterParams) {
    ...
    const lzTxParams = {
      ...
      dstNativeAddr: (_a = adapterParams.dstNativeAddress) != null ? _a : uiEvm.ONE_ADDRESS,
      dstNativeAmount: adapterParams.dstNativeAmount ? uiCore.toBigNumber(adapterParams.dstNativeAmount) : ethers.constants.Zero
    };
    return lzTxParams;
  }
  ...
  async getOutput(inputAmountLD, dstCurrency) {
    ...
    const fees = await feeLibrary.getFees(
      ...
      dstPool.chainId,
      srcAddress,
      uiCore.toBigNumber(amountSD)
    );
    const feeSD = {
      eqFee: uiCore.toCurrencyAmount(srcPool.lpToken, fees.eqFee),
      eqReward: uiCore.toCurrencyAmount(srcPool.lpToken, fees.eqReward),
      lpFee: uiCore.toCurrencyAmount(srcPool.lpToken, fees.lpFee),
      protocolFee: uiCore.toCurrencyAmount(srcPool.lpToken, fees.protocolFee),
      lkbRemove: uiCore.toCurrencyAmount(srcPool.lpToken, fees.lkbRemove)
    };
    const feeLD = {
  ...
  async getLimit(srcCurrency, dstCurrency) {
    ...
    const balanceLP = uiCore.toCurrencyAmount(srcPool.lpToken, chainPath.balance);
  ...
  async getDuration(srcCurrency, dstCurrency) {
    return uiCore.getDefaultMessageDuration(srcCurrency.chainId, dstCurrency.chainId);
  }
};
...
var WIDGET_SWAP_ADDRESS = {
  [lzSdk.ChainId.RINKEBY]: "0x1aE3A80BDCdFEC8c76c6049248E4139160b72FF1",
  [lzSdk.ChainId.BSC_TESTNET]: "0xcE375F5d95204DB9b110BE3b3C886A33c8168eB2",
  [lzSdk.ChainId.FUJI]: "0xcE375F5d95204DB9b110BE3b3C886A33c8168eB2",
  [lzSdk.ChainId.MUMBAI]: "0xcE375F5d95204DB9b110BE3b3C886A33c8168eB2",
  [lzSdk.ChainId.ARBITRUM_RINKEBY]: "0xcE375F5d95204DB9b110BE3b3C886A33c8168eB2",
  [lzSdk.ChainId.OPTIMISM_KOVAN]: "0xcE375F5d95204DB9b110BE3b3C886A33c8168eB2",
  [lzSdk.ChainId.FANTOM_TESTNET]: "0xcbF0B97dbe30406e974F2d1ADF19CD6667d70632",
  [lzSdk.ChainId.ETHEREUM]: "0x76d4d68966728894961AA3DDC1d5B0e45668a5A6",
  [lzSdk.ChainId.BSC]: "0x2Eb9ea9dF49BeBB97e7750f231A32129a89b82ee",
  [lzSdk.ChainId.AVALANCHE]: "0x20293eDD4f52F81234b3997B9AE4742c48005858",
  [lzSdk.ChainId.POLYGON]: "0xdc2716B92480225533aBC3328C2Ab961f2A9247d",
  [lzSdk.ChainId.ARBITRUM]: "0x6c33A7b29C8B012D060F3a5046f3ee5aC48f4780",
  [lzSdk.ChainId.OPTIMISM]: "0x46Bc16F76B0aE14Abb820D3410843Ba54D8ef6f0",
  [lzSdk.ChainId.FANTOM]: "0xC8e5157EC44E00ff85Bf15D4f50974d3A8166427",
  [lzSdk.ChainId.METIS]: "0xb63c87D146cbE60B3C0419003Ebd24F21374c8Ae"
};
...
var StargateWidgetBridge__evm = class extends StargateBridge__evm {
  ...
  async approve(amount) {
    ...
    assert__default.default(widgetAddress, "widgetAddress");
  ...
  async transferEth(input) {
    ...
    const value = uiCore.toBigNumber(input.fee.nativeFee.add(input.amount));
    const populatedTransaction = contract.populateTransaction.swapETH(
      uiCore.toBigNumber(input.amount),
      uiCore.toBigNumber(input.minAmount),
      input.dstChainId,
      input.dstAddress,
      partnerId,
      this.feeObj,
    ...
    return uiEvm.createTransaction(populatedTransaction, { provider: contract.provider });
  }
  get feeObj() {
    const feeObj = {
  ...
  async transferToken(input) {
    ...
    const lzTxParams = {
      dstGasForCall: 0,
      dstNativeAmount: input.adapterParams.dstNativeAmount ? uiCore.toBigNumber(input.adapterParams.dstNativeAmount) : 0,
  ...
  getWidgetContract(chainId) {
    ...
    assert__default.default(address, "WIDGET_SWAP_ADDRESS");
...
var StargateSDK = class {
  constructor(config = mainnet) {
    ...
    this.swapGroups = [[sgSdk.PoolId.BUSD, sgSdk.PoolId.USDC, sgSdk.PoolId.USDT]];
    ...
    this.getRouterAddress = (chainId) => {
      ...
      assert__default.default(address, "No router address");
      return address;
    };
    this.getRouterEthAddress = (chainId) => {
      const address = this.tryGetRouterEthAddress(chainId);
      assert__default.default(address, "No router ETH address");
      return address;
    };
  }
  // other
...
function isMetis(chainId) {
  return chainId === lzSdk.ChainId.METIS || chainId === lzSdk.ChainId.METIS_TESTNET;
}

exports.StargateBridge__evm = StargateBridge__evm;
exports.StargateSDK = StargateSDK;
exports.StargateWidgetBridge__evm = StargateWidgetBridge__evm;
